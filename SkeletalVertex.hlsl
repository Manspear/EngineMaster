...
VS_OUT VSSkeletal_main(VS_IN input)
{
	...
	output.Pos = float4(0.f, 0.f, 0.f, 0.f); //typecasting float3 to float4. The 1 is the w-part of the 4x1 point
	output.normal = float4(0.f, 0.f, 0.f, 0.f);

	for (int i = 0; i < 4; i++)
	{
		output.Pos += mul(mul(jointTransform[input.influences[i]], float4(input.Pos, 1.0)), input.weights[i]);
		output.normal += mul(mul(jointTransform[input.influences[i]], float4(input.normal, 1.0)), input.weights[i]);
	}
	//The worldmatrix is already calculated on the CPU into the jointTransform[36], so it shouldn't be needed here.
	
	//see mesh but its scrambled
